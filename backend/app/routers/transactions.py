...
async def upload_pdf(
    ...
    # Ask OpenAI to extract transactions
    import json
    import openai
    ...
    prompt = (
        f"Extract all transactions from this bank statement ({extraction_mode}) as a JSON object with key 'transactions' containing a list of objects.\n"
...
async def auto_categorize_tax(
    ...
    prompt = (
        ...
        f"Return a JSON object with key 'classifications' containing a list of objects: {{\"id\": int, \"tax_category\": str, \"is_deductible\": bool}}.\n\n"
        f"Transactions:\n{json.dumps(txn_list)}"
    )
    ...
    try:
        ...
        raw = response.choices[0].message.content.strip()
        parsed = json.loads(raw)
        categorized = parsed.get("classifications", [])
...
async def get_anomalies(
    ...
    prompt = (
        ...
        "Return a JSON object with key 'anomalies' containing a list of objects with fields: "
