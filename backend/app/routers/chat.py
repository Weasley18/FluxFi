...
async def get_insights(
    ...
    response = client.chat.completions.create(
        ...
        messages=[{
            ...
            "content": (
                ...
                "Return exactly 3 actionable insights as a JSON object with a key 'insights' containing an array of strings."
            ),
        }],
    )
    ...
    try:
        ...
        if isinstance(parsed, dict) and "insights" in parsed:
            insights = [str(i).strip() for i in parsed["insights"] if str(i).strip()]
