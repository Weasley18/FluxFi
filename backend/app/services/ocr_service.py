...
def parse_invoice_fields(text: str) -> dict:
    """Extract structured fields from raw OCR text using OpenAI, with regex fallback."""
    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        logger.warning("OPENAI_API_KEY not set, falling back to regex parser")
    ...
    try:
        ...
        logger.error(f"OpenAI invoice parsing failed: {exc}, falling back to regex")
        return _parse_invoice_fields_regex(text)
